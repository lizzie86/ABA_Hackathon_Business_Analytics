...
```
결론적으로, 테스트에러가 트레이닝에러보다 낮은, 그리고 baseline model보다 peformance가 좋은 모델을 찾게 되었다. 

이 모델이 좋은 성능을 보인다고 단정 짓기에는 몇 가지 의문점이 있습니다. 테스트 세트에서의 낮은 에러는 긍정적인 신호일 수 있지만, 밸리데이션 세트의 높은 에러와 트레이닝 세트 대비 테스트 세트의 낮은 에러는 데이터 세트의 분포나 품질, 그리고 가능한 과적합 문제에 대해 추가적인 검토가 필요함을 나타냅니다.

흡연자 데이터를 오버샘플링한 것이 트레이닝, 밸리데이션, 그리고 테스트 세트에서의 에러 메트릭에 영향을 미쳤을 가능성이 있습니다. 
오버샘플링은 특정 클래스의 데이터가 반복되어 사용됨으로써 모델이 이러한 반복되는 패턴에 과적합될 수 있습니다. 이 경우, 모델은 훈련 데이터에 대해서는 잘 작동하지만, 새로운 데이터, 즉 밸리데이션 및 테스트 세트에서는 일반화를 잘 하지 못할 수 있습니다.
테스트 세트의 대표성: 오버샘플링은 트레이닝 및 밸리데이션 세트에만 적용되어야 합니다. 테스트 세트는 가능한 한 실제 환경의 데이터 분포를 반영해야 하므로, 오버샘플링된 데이터를 포함해서는 안 됩니다. 
